...
def send_async_verification_email(user_id, new_email, verification_link):
    ...
    try:
        ...
        message = render_to_string('accounts/account_change_email.html', {
            'account': user,
...
def task_matchmake_node(node_id):
    ...
    with transaction.atomic():
        ...
        current_pending_count = node.reviews.filter(
            round_number=node.revision_count,
            status='pending'
        ).count()
        ...
        past_firm_reviews = list(node.reviews.filter(
            status__in=['claimed', 'completed']
        ).values_list('assigned_reviewer_id', flat=True))

        # Exclude anyone who has already been offered a spot in the CURRENT round
        current_round_offers = list(node.reviews.filter(
            round_number=node.revision_count
        ).values_list('assigned_reviewer_id', flat=True))
        ...
        base_eligible = Agent.objects.filter(is_active=True).exclude(
            maintainer__username='Public_Pool'
        ).exclude(
            maintainer__username=TREASURY_USERNAME
        ).exclude(
            maintainer_id__in=involved_maintainer_ids
        ...
        active_ids = list(base_eligible.filter(last_active_at__gte=recent_cutoff).values_list('id', flat=True).distinct())
        ...
        if len(active_ids) >= to_assign:
            ...
            # Find inactive ones
            inactive_ids = list(base_eligible.filter(last_active_at__lt=recent_cutoff).values_list('id', flat=True).distinct())
        ...
        if selected_ids:
            ...
            for reviewer in selected_reviewers:
                ...
                logger.info(f"Broadcasted PeerReview offer to Agent {reviewer.name} for Node: {node.title} (Round {node.revision_count})")
        else:
...
def task_matchmake_counsel(node_id):
    ...
    with transaction.atomic():
        try:
            node = ResearchNode.objects.select_for_update().get(id=node_id, status='in_review', escalated_to_counsel=True)
        except ResearchNode.DoesNotExist:
            return
        ...
        base_query = Agent.objects.filter(is_active=True).exclude(
            maintainer__username='Public_Pool'
        ).exclude(
            maintainer__username=TREASURY_USERNAME
        ).exclude(
...
def process_reviewer_rewards(node, round_number, is_approved_ground_truth):
    ...
    # Determine base fee
    base_fee = COUNSEL_BS_REWARD if (node.escalated_to_counsel and round_number == node.revision_count) else REVIEWER_BS_REWARD
    ...
    for review in reviews:
        ...
        updated_rows = Account.objects.filter(
            ...
            balance_blue_stars__gte=base_fee
        ).update(balance_blue_stars=F('balance_blue_stars') - base_fee)
        ...
        if review.is_approved == is_approved_ground_truth:
            ...
            updated_bonus_rows = Account.objects.filter(
                username=TREASURY_USERNAME, 
                balance_blue_stars__gte=accuracy_bonus_bs
...
def execute_publish(node):
    from main_api.models import ResearchNode, Paper, PeerReview
    from accounts.models import Agent, Account
    from social.models import Notification
    ...
    if fulfilling_agents.exists():
        ...
        Account.objects.filter(username=TREASURY_USERNAME).update(
            balance_blue_stars=F('balance_blue_stars') + tax
        )
        ...
        for agent in fulfilling_agents:
            ...
            Account.objects.filter(id=agent.maintainer_id).update(balance_blue_stars=F('balance_blue_stars') + total_payout)
...
def task_resolve_node(node_id):
    ...
    try:
        with transaction.atomic():
            ...
            completed_reviews_qs = node.reviews.filter(round_number=node.revision_count, structured_data__isnull=False).select_related('assigned_reviewer')
            ...
            if relative_strength < 0.4: strength = 'Marginal'
            elif relative_strength < 0.8: strength = 'Clear'
            ...
            if node.escalated_to_counsel:
                ...
                if is_approved: execute_publish(node)
                else: execute_reject(node)
            else:
...
def task_sweep_deadlines():
    ...
    expired_nodes = ResearchNode.objects.filter(
        ...
        deadline__lt=timezone.now() - timedelta(minutes=2)
...
def task_sweep_stale_reviews():
    ...
    try:
        ...
        with transaction.atomic():
            ...
            stale_pending_ids = list(PeerReview.objects.filter(
                status='pending',
                created__lt=pending_timeout
            ).values_list('id', flat=True))

            stale_claimed_ids = list(PeerReview.objects.filter(
                status='claimed',
                claimed_at__lt=claimed_timeout
            ).values_list('id', flat=True))
            ...
            if not stale_review_ids:
                ...
                affected_node_ids = list(PeerReview.objects.filter(id__in=stale_review_ids).values_list('research_node_id', flat=True).distinct())
                from main_api.models import ResearchNode
                locked_nodes = list(ResearchNode.objects.select_for_update().filter(id__in=affected_node_ids).order_by('id'))
                
                # Step 3: Now lock the specific stale reviews
                stale_pending = list(PeerReview.objects.select_for_update().filter(id__in=stale_pending_ids).order_by('id'))
                stale_claimed = list(PeerReview.objects.select_for_update().filter(id__in=stale_claimed_ids).order_by('id'))
                stale_reviews = stale_pending + stale_claimed

                logger.info(f"Purging {len(stale_reviews)} stale reviews (Pending: {len(stale_pending)}, Claimed: {len(stale_claimed)})")
            ...
            deleted_count, _ = PeerReview.objects.filter(
                status__in=['rejected', 'aborted'],
                created__lt=dead_timeout
            ).delete()
            if deleted_count > 0:
...
def task_fill_counsel_shortages():
    ...
    for node in counsel_nodes:
        ...
        if firm_reviews < 5:
            logger.info(f"Counsel shortage on Node {node.id} ({firm_reviews}/5 firm reviews). Re-triggering matchmaking.")
