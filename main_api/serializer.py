...
from .models import Capability, ResearchNode, Comment, SubComment, ProfaneWord, PeerReview, ResearchKeyword, Paper, AgentDirective, NodeType, Bid, Attachment, AgentMessage
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'username', 'rank', 'score', 'avatar', 'average_rating','average_soundness', 'average_novelty', 'average_significance', 'average_clarity', 'total_ratings', 'total_fulfillments', 'total_research_nodes', 'total_saves', 'follower_count', 'total_visits', 'biography', 'joined_date', 'total_appreciation_score', 'saved_papers', 'active_agents']
...
class CapabilitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'title', 'slug', 'description', 'icon', 
                  'parent_capabilities', 'child_capabilities', 'has_children']
...
class AgentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'name', 'maintainer', 'maintainer_id', 'orange_stars', 'is_active', 'is_online', 'capabilities', 'capabilities_detail', 'api_key_hash', 'created_at']
...
class ResearchNodeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ResearchNode
        fields = ['id', 'title', 'description', 'body', 'required_capabilities', 'keywords', 'type', 'status',
                  'bounty_amount', 'required_reviews', 'required_collaborators', 'min_trust_required', 'research_duration_days', 'deadline', 'coordinating_agent', 'updated', 'visits',
                  'saves', 'average_rating','average_soundness', 'average_novelty', 'average_significance', 'average_clarity', 'total_ratings', 'total_assigned',
                  'orchestrator_verdict', 'verdict_strength', 'revision_count', 'escalated_to_counsel', 'decision_deadline', 'interview_prompt', 'coordination_plan']
    ...
    def to_representation(self, instance):
        ...
        if instance.status == 'published' or instance.status == 'open':
            ...
            if isinstance(user, Agent):
                if instance.coordinating_agent == user or \
                   instance.assigned_agents.filter(id=user.id).exists() or \
                   instance.reviews.filter(assigned_reviewer=user).exists():
                    show_body = True
            elif user.is_authenticated:
                if (instance.coordinating_agent and instance.coordinating_agent.maintainer == user) or \
                   instance.assigned_agents.filter(maintainer=user).exists() or \
                   instance.reviews.filter(assigned_reviewer__maintainer=user).exists():
                    show_body = True
...
class ResearchNodeCardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ResearchNode
        fields = ['id', 'title', 'description', 'required_capabilities', 'keywords', 'type', 'status',
                  'bounty_amount', 'required_collaborators', 'deadline', 'coordinating_agent',
                  'saves', 'average_rating', 'total_ratings', 'total_assigned']
        
    def get_coordinating_agent(self, obj):
        return {"name": obj.coordinating_agent.name, "id": obj.coordinating_agent.id} if obj.coordinating_agent else None
...
class PaperSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Paper
        fields = ['id', 'research_node', 'research_node_title', 'title', 'abstract', 'content', 'published_date', 'authors', 'appreciation_score', 'user_vote', 'saves']
...
class AgentDirectiveSerializer(serializers.ModelSerializer):
    ...
    agent = serializers.PrimaryKeyRelatedField(queryset=Agent.objects.all(), write_only=True, required=False, allow_null=True)
    maintainer_username = serializers.ReadOnlyField(source='maintainer.username')

    class Meta:
        model = AgentDirective
        fields = ['id', 'maintainer', 'maintainer_username', 'agent', 'agent_detail', 'content', 'agent_response', 'status', 'created_at', 'updated_at']
...
class PeerReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PeerReview
        fields = ['id', 'assigned_reviewer', 'assigned_reviewer_detail', 'research_node', 'research_node_detail', 'value', 'soundness', 'significance', 'novelty', 'clarity', 'confidence', 'detailed_comments', 'recommendation', 'is_approved', 'structured_data', 'status', 'claimed_at', 'round_number']
        read_only_fields = ['id', 'assigned_reviewer', 'research_node', 'created', 'value', 'is_approved', 'claimed_at', 'round_number']
...
class CreateResearchNodeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ResearchNode
        fields = ['id', 'title', 'description', 'body', 'required_capabilities', 'keywords', 'type', 'status', 'bounty_amount', 'required_reviews', 'required_collaborators', 'min_trust_required', 'research_duration_days', 'deadline', 'interview_prompt']
    ...
    def create(self, validated_data):
        ...
        for kw_name in keywords_data:
            ...
            kw_obj, _ = ResearchKeyword.objects.get_or_create(
                slug=kw_slug,
                defaults={'name': kw_name}
            )
...
class EditResearchNodeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['title', 'description', 'body', 'required_capabilities', 'keywords', 'type', 'required_reviews', 'required_collaborators', 'min_trust_required', 'interview_prompt']
...
class AgentSyncNodeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'title', 'description', 'required_capabilities', 'keywords', 'type', 'status',
                  'bounty_amount', 'required_collaborators', 'deadline', 'coordinating_agent',
                  'research_duration_days', 'interview_prompt', 'coordination_plan']
