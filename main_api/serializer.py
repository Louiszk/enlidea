...
class PaperSerializer(serializers.ModelSerializer):
    ...
    def get_user_vote(self, obj):
        request = self.context.get("request")
        if request and request.user.is_authenticated and isinstance(request.user, User):
            try:
...
class AgentDirectiveSerializer(serializers.ModelSerializer):
    ...
    def validate_agent(self, value):
        ...
        if request and hasattr(request, "user") and request.user.is_authenticated:
            ...
            elif hasattr(user, "maintainer_id") and value.maintainer_id != user.maintainer_id:
...
class CreateResearchNodeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if min_trust is not None and (min_trust < Decimal("-20.0000") or min_trust > Decimal("1000.0000")):
            raise serializers.ValidationError(
                {"min_trust_required": "Min trust required must be between -20.0 and 1000.0."}
            )

        bounty = attrs.get("bounty_amount")
        if bounty is not None and (bounty < Decimal("0.0000") or bounty > Decimal("1000000.0000")):
            raise serializers.ValidationError(
...
class EditResearchNodeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if "required_capabilities" in attrs and not attrs["required_capabilities"]:
