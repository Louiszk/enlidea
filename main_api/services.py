...
from .models import ResearchNode, Bid, AgentMessage, Attachment, Paper, ResearchKeyword
...
def download_remote_file(url, max_size_bytes, allowed_extensions=None):
    ...
    if not hostname or hostname.lower() not in ALLOWED_DOMAINS:
        raise ValidationError(
            f"Access Denied: Domain '{hostname}' is not in the approved allowlist."
        )
...
def create_research_node(agent, validated_data):
    ...
    with transaction.atomic():
        ...
        if maintainer.balance_blue_stars < total_cost:
            raise DRFValidationError({"detail": f"Insufficient Blue Stars. Creating a node costs {creation_fee} BS fee plus the bounty of {bounty} BS."})
...
def update_research_node(node, user, validated_data):
    ...
    with transaction.atomic():
        ...
        if external_bids or locked_node.status != 'open':
            raise DRFValidationError({"detail": "Cannot edit a node that has active external bids or is no longer open."})
...
def delete_research_node(node, user):
    with transaction.atomic():
        ...
        if external_bids or locked_instance.status != 'open':
            raise DRFValidationError({"detail": "Cannot delete a node that has active external bids or is no longer open."})
        ...
        if locked_instance.assigned_agents.exists():
            stake_amount = max(Decimal('2.0000'), (locked_instance.bounty_amount * Decimal('0.10')).quantize(Decimal('0.0001')))
            maintainer_refunds = {}
            for agent in locked_instance.assigned_agents.all():
                maintainer_refunds[agent.maintainer_id] = maintainer_refunds.get(agent.maintainer_id, Decimal('0.0000')) + stake_amount
            
            for m_id, amount in maintainer_refunds.items():
                User.objects.filter(id=m_id).update(
                    balance_blue_stars=F('balance_blue_stars') + amount
                )
    ...
    with transaction.atomic():
        ...
        if node.bounty_amount > 0:
            ...
            if agent.orange_stars < min_trust:
                raise PermissionDenied(f'Insufficient trust score to bid on paid bounties. Need at least {min_trust} OS.')
...
def handle_coordinator_decision(user, node, action_choice):
    with transaction.atomic():
        ...
        from .tasks import (
            execute_publish, execute_reject, task_handle_node_deadline, 
            task_matchmake_counsel, process_reviewer_rewards,
            REVISION_FEE, ESCALATION_FEE, TREASURY_USERNAME
        )
        ...
        if action_choice == 'publish':
            ...
            maintainer.save()
            User.objects.filter(username=TREASURY_USERNAME).update(balance_blue_stars=F('balance_blue_stars') + REVISION_FEE)
            ...
            transaction.on_commit(lambda n_id=locked_node.id, n_eta=locked_node.deadline: task_handle_node_deadline.apply_async(args=[n_id], eta=n_eta))
            ...
            eligible_pool_count = Agent.objects.filter(is_active=True).exclude(
                ...
                id__in=excluded_agent_ids
            ).count()

            if eligible_pool_count < 5:
                raise DRFValidationError({'detail': f'Cannot escalate: Only {eligible_pool_count} eligible agents found on the network (need 5).'})

            if maintainer.balance_blue_stars < ESCALATION_FEE:
                raise DRFValidationError({'detail': f'Insufficient funds for escalation ({ESCALATION_FEE} BS required).'})
            
            maintainer.balance_blue_stars -= ESCALATION_FEE
            maintainer.save()
            User.objects.filter(username=TREASURY_USERNAME).update(balance_blue_stars=F('balance_blue_stars') + ESCALATION_FEE)
