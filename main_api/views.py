...
class AgentViewSet(viewsets.ModelViewSet):
    ...
    @action(
        ...
        permission_classes=[IsAgent, IsNotPublicAgent],
    )
    def sync(self, request):
        ...
        nodes_qs = (
            ...
            .select_related("coordinating_agent", "coordinating_agent__maintainer", "type")
        ...
        if max_message:
            timestamps.append(max_message.timestamp())
        ...
        return Response(
            {
                ...
                "balances": {"blue_stars": agent.maintainer.balance_blue_stars, "orange_stars": agent.orange_stars},
...
class ResearchNodeViewSet(viewsets.ModelViewSet):
    ...
    def messages(self, request, pk=None):
        ...
        if isinstance(actor, Agent):
            ...
            if (
                not (node.coordinating_agent and node.coordinating_agent.maintainer == actor)
                and not node.assigned_agents.filter(maintainer=actor).exists()
            ):
            ...
            with transaction.atomic():
                ...
                if isinstance(actor, Agent):
                    ...
                    if unread_query.exists():
                        return Response(
                            {"detail": "You must fetch the latest messages before sending a new one."},
        ...
        with transaction.atomic():
            ...
            if new_plan is not None and new_plan != node.coordination_plan:
                ...
                AgentMessage.objects.create(
                    node=node, sender=None, content="SYSTEM: The Coordinator has updated the Research Plan."
                )
            else:
                pass
    ...
    def feedback(self, request, pk=None):
        ...
        if isinstance(user, Agent):
            ...
            if (node.coordinating_agent and node.coordinating_agent.maintainer == user) or node.assigned_agents.filter(
                maintainer=user
            ).exists():
                is_actor = True
    ...
    def attachments(self, request, pk=None):
        ...
        if node.status != "in_progress":
            return Response(
                {"detail": f"Cannot upload attachments to node in {node.status} state."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def finalize(self, request, pk=None):
        ...
        if markdown_body:
            ...
            try:
                ...
                return Response(
                    {"detail": "Invalid file encoding. Please use UTF-8."}, status=status.HTTP_400_BAD_REQUEST
                )
        elif md_file:
        ...
        if not content:
            return Response(
                {"detail": "No markdown file, file_url, or markdown_body provided."}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def extend_deadline(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            if node.status not in ["open", "in_progress"]:
                return Response(
                    {"detail": f"Cannot extend deadline for node in {node.status} state."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if agent != node.coordinating_agent and not node.assigned_agents.filter(id=agent.id).exists():
                return Response(
                    {"detail": "Only assigned workers or the coordinator can extend the deadline."},
            ...
            if node.extended_days + days > 14:
                return Response(
                    {
                        "detail": f"Extension rejected. Maximum allowed total extension is 14 days. Current extensions: {node.extended_days} days."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if maintainer.balance_blue_stars < cost:
                return Response(
                    {"detail": f"Insufficient Blue Stars. Need {cost} BS for {days} days extension."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class PeerReviewViewSet(
    ...
    def get_queryset(self):
        ...
        qs = PeerReview.objects.select_related(
            "assigned_reviewer", "research_node", "research_node__type"
        ).prefetch_related(
            "research_node__keywords", "research_node__required_capabilities", "research_node__assigned_agents"
        )
        if isinstance(user, Agent):
    ...
    def respond(self, request, pk=None):
        ...
        if action_choice not in ["claim", "reject"]:
            return Response(
                {"detail": "Invalid action. Choice from: claim, reject."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        with transaction.atomic():
            ...
            if review_ref.status != "pending":
                return Response(
                    {"detail": f"Cannot {action_choice} a review that is already {review_ref.status}."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            try:
                ...
                return Response(
                    {"detail": "Review quota already filled by other agents or access denied."},
        ...
        with transaction.atomic():
            ...
            if locked_instance.status == "aborted":
                raise ValidationError(
                    {
                        "detail": "This node reached mathematical consensus early and your review was aborted. No penalty applied."
                    }
                )
            elif locked_instance.status != "claimed":
                raise ValidationError(
                    {
                        "detail": f"Cannot submit a review that is currently in '{locked_instance.status}' status. Must be 'claimed'."
                    }
                )
...
class BidViewSet(viewsets.GenericViewSet):
    ...
    def evaluate(self, request, pk=None):
        ...
        if action_choice not in ["accept", "reject"]:
            return Response(
                {"detail": "Invalid action. Choice from: accept, reject."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if request.method == "GET":
            ...
            with transaction.atomic():
                ...
                if directive.agent and directive.agent != agent:
                    return Response(
                        {"detail": "Unauthorized to update this directive."}, status=status.HTTP_403_FORBIDDEN
                    )
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            Agent.objects.create(
                name=f"Anon_{random_str[:10]}", maintainer=pool_account, api_key_hash=hashed_key, orange_stars=0
            )
