...
class OrchestratorConsensusTest(TestCase):
    ...
    def test_matchmaking_assigns_multiple_reviewers(self):
        ...
        reviewer_maintainer = Account.objects.create(
            username='reviewer_maintainer',
            email='rev@enlidea.com',
            balance_blue_stars=1000
        )
        # Create more agents
        for i in range(5):
            a = Agent.objects.create(
                name=f'Reviewer_{i}',
                maintainer=reviewer_maintainer,
    ...
    def test_matchmaking_no_required_capabilities(self):
        ...
        # Create a DIFFERENT maintainer for reviewers (Collusion Prevention)
        reviewer_maintainer = Account.objects.create(
            username='general_reviewer_maintainer',
            email='genrev@enlidea.com',
            balance_blue_stars=1000
        )

        # Create exactly ONE eligible reviewer
        Agent.objects.create(
            name='General_Reviewer',
            maintainer=reviewer_maintainer,
            api_key_hash='gen_hash'
        )
    ...
    def test_resolution_consensus_published(self):
        ...
        # Create a DIFFERENT maintainer for reviewers (Collusion Prevention)
        reviewer_maintainer = Account.objects.create(
            username='published_reviewer_maintainer',
            email='pubrev@enlidea.com',
            balance_blue_stars=1000
        )
        # Assign 3 reviewers manually
        reviewers = []
        for i in range(3):
            r = Agent.objects.create(
                name=f'Rev_{i}',
                maintainer=reviewer_maintainer,
                api_key_hash=f'revhash_{i}'
            )
            reviewers.append(r)
            PeerReview.objects.create(
                research_node=self.node,
                assigned_reviewer=r, status='completed',
    ...
    def test_resolution_consensus_rejected(self):
        ...
        for i in range(3):
            r = Agent.objects.create(
                name=f'RevFail_{i}',
                maintainer=self.maintainer,
                api_key_hash=f'revfailhash_{i}'
            )
            PeerReview.objects.create(
                research_node=self.node,
                assigned_reviewer=r, status='completed',
                soundness=1, significance=1, novelty=1, clarity=1, value=1.0,
                is_approved=(i == 0),
