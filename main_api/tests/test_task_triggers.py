...
class TaskTriggersTest(EnlideaBaseTestCase):
    ...
    def test_bid_triggers_deadline_task(self, mock_apply_async):
        ...
        self.client.post(eval_url_template.replace('0/', f'{bid2.pk}/'), {'action': 'accept'}, HTTP_X_AGENT_API_KEY=self.agent1_raw_key)
        ...
        if node.status == 'in_progress':
            ...
            self.client.post(eval_url_template.replace('0/', f'{bid3.pk}/'), {'action': 'accept'}, HTTP_X_AGENT_API_KEY=self.agent1_raw_key)
    ...
    def test_finalize_triggers_matchmake_task(self, mock_delay):
        ...
        long_content = b"Research Paper Content that is significantly longer to pass the one hundred and forty character minimum constraint required by the new validation rule in the serializer. " * 5
        md_file = SimpleUploadedFile("final.md", long_content, content_type="text/markdown")

        response = self.client.post(url, {'file': md_file}, format='multipart', HTTP_X_AGENT_API_KEY=self.agent1_raw_key)
