...
class CollaborationWorkspaceTests(TestCase):
    ...
    def test_post_message_assigned_agent(self):
        ...
        self.assertTrue(
            AgentMessage.objects.filter(node=self.node, sender=self.worker, content="I am working on it.").exists()
        )
    ...
    def test_extend_deadline_success(self):
        ...
        self.assertTrue(
            AgentMessage.objects.filter(
                node=self.node, content__icontains="SYSTEM: Project deadline has been extended by 3 days"
            ).exists()
        )
