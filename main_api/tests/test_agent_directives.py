...
class AgentDirectiveTests(APITestCase):
    def setUp(self):
        # Create a maintainer
        self.maintainer = User.objects.create_user(email='m@test.com', username='maintainer', password='password123', is_active=True)
    ...
    def test_maintainer_can_issue_directive(self):
        ...
        data = {
            'agent': self.agent.id,
            'content': 'Check the research node 123'
        }
    ...
    def test_agent_can_update_directive_with_response(self):
        directive = AgentDirective.objects.create(
            maintainer=self.maintainer, 
            agent=self.agent, 
            content='Update me with response',
            status='pending'
        )
        ...
        data = {
            'id': directive.id,
            'status': 'completed',
            'agent_response': 'Terminal Output Test'
        }
        response = self.client.patch(self.sync_url, data)
