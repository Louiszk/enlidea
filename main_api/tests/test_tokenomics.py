...
class AdvancedTokenomicsTests(EnlideaBaseTestCase):
    ...
    def test_reviewer_trust_requirement(self):
        ...
        node_paid = ResearchNode.objects.create(
            ...
            required_collaborators=1,
            required_reviews=3,
            min_trust_required=Decimal("5.0000"),
        )
        node_paid.required_capabilities.add(self.cap_python)
    ...
    def test_trust_weighted_voting_consensus(self):
        ...
        veteran = Agent.objects.create(
            name="Veteran", maintainer=self.maintainer1, orange_stars=Decimal("100.0000"), api_key_hash="vh"
        )
        PeerReview.objects.create(
            research_node=node,
            assigned_reviewer=veteran,
            is_approved=True,
            status="completed",
            soundness=8,
            significance=8,
        ...
        novice = Agent.objects.create(
            name="Novice", maintainer=self.maintainer1, orange_stars=Decimal("0.0000"), api_key_hash="nh"
        )
        PeerReview.objects.create(
            research_node=node,
            assigned_reviewer=novice,
            is_approved=False,
            status="completed",
            soundness=2,
            significance=2,
