...
class NodeLifecycleTests(EnlideaBaseTestCase):
    def test_successful_bid_and_status_progression(self):
        ...
        # Agent 2 bids
        response = self.client.post(bid_url, {'interview_response': 'I am an expert in Python.'}, HTTP_X_AGENT_API_KEY=self.agent2_raw_key)
    ...
    def test_attachments_and_finalize(self):
        ...
        response = self.client.post(url_attach, {'file': image_file}, format='multipart', HTTP_X_AGENT_API_KEY=self.agent2_raw_key)
        ...
        md_content = f"Research Paper\n\n![Image]({attachment_url})\n\n" + "This is a very long research paper content that exceeds the minimum character limit of one hundred and forty characters imposed by the body serializer validation. " * 5
        md_file = SimpleUploadedFile("final.md", md_content.encode('utf-8'), content_type="text/markdown")

        response = self.client.post(url_finalize, {'file': md_file}, format='multipart', HTTP_X_AGENT_API_KEY=self.agent1_raw_key)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_finalize_with_image_title(self):
        ...
        response = self.client.post(url_attach, {'file': image_file}, format='multipart', HTTP_X_AGENT_API_KEY=self.agent1_raw_key)
        attachment_url = response.data['url']
...
class OrchestratorTests(EnlideaBaseTestCase):
    def test_resolution_and_payouts(self):
        ...
        PeerReview.objects.create(
            research_node=node,
            assigned_reviewer=reviewer, status='completed',
    ...
    def test_collaborative_payout_split(self):
        ...
        m_worker1 = Account.objects.create(username='mw1', email='mw1@enlidea.com', balance_blue_stars=Decimal('1000.0000'))
        m_worker2 = Account.objects.create(username='mw2', email='mw2@enlidea.com', balance_blue_stars=Decimal('1000.0000'))
        m_worker3 = Account.objects.create(username='mw3', email='mw3@enlidea.com', balance_blue_stars=Decimal('1000.0000'))
        ...
        m_rev = Account.objects.create(username='m_rev', email='mrev@enlidea.com', balance_blue_stars=Decimal('1000.0000'))
