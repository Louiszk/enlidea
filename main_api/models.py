...
class Capability(models.Model):
    ...
    child_capabilities = models.ManyToManyField('self', blank=True, symmetrical=False, related_name='parent_capabilities')
    ...
    def get_trend_score(self, time_range=365):
        ...
        return ResearchNode.with_trend_score().filter(
            required_capabilities=self,
            created__gte=one_year_ago
        ).aggregate(total_trend_score=Sum('trend_score', distinct=True))['total_trend_score'] or 0
...
class ResearchNode(models.Model):
    ...
    bounty_amount = models.DecimalField(max_digits=12, decimal_places=4, default=0.0000, validators=[MinValueValidator(Decimal('0'))])
    forfeited_bounty = models.DecimalField(max_digits=12, decimal_places=4, default=0.0000, help_text="Bounty shares forfeited by auto-kicked workers.")
    ...
    created = models.DateTimeField(auto_now_add=True)
    coordinating_agent = models.ForeignKey('accounts.Agent', on_delete=models.CASCADE, related_name='coordinated_nodes', null=True)
    ...
    def with_trend_score(cls):
        return cls.objects.annotate(
            trend_score=ExpressionWrapper(
                F('trend__daily_visits__0') + F('trend__daily_visits__1') + 
                F('trend__daily_visits__2') + F('trend__daily_visits__3') + 
                F('trend__daily_visits__4') + F('trend__daily_visits__5') + 
                F('trend__daily_visits__6') +
                2 * (F('trend__daily_saves__0') + F('trend__daily_saves__1') + 
                     F('trend__daily_saves__2') + F('trend__daily_saves__3') + 
                     F('trend__daily_saves__4') + F('trend__daily_saves__5') + 
    ...
    def with_fulfillment_score(cls):
        return cls.objects.annotate(
            fulfillment_score=ExpressionWrapper(
                F('trend__daily_fulfillments__0') + F('trend__daily_fulfillments__1') + 
                F('trend__daily_fulfillments__2') + F('trend__daily_fulfillments__3') + 
                F('trend__daily_fulfillments__4') + F('trend__daily_fulfillments__5') + 
...
class PeerReview(models.Model):
    assigned_reviewer = models.ForeignKey('accounts.Agent', on_delete=models.CASCADE, related_name='reviews_given', null=True)
    ...
    value = models.DecimalField(max_digits=12, decimal_places=4, default=5.0000, validators=[MinValueValidator(Decimal('0')), MaxValueValidator(Decimal('10'))])
...
class AgentDirective(models.Model):
    ...
    agent = models.ForeignKey('accounts.Agent', on_delete=models.CASCADE, related_name='directives_received', null=True, blank=True)
...
class AgentMessage(models.Model):
    ...
    sender = models.ForeignKey('accounts.Agent', on_delete=models.SET_NULL, null=True, blank=True, related_name='sent_messages')
