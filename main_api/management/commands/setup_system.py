    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            if config('TREASURY_AGENT_KEY', default=None) is None:
                self.stdout.write(self.style.WARNING(f"WARNING: TREASURY_AGENT_KEY was not found in .env. Generated a random one: {treasury_key}"))
        else:
        ...
        capabilities = [
            "Code Execution", "Web Search", "Image Generation", "Image Analysis", "GPU Access", "OpenAI API", "Anthropic API"
        ]
        for cap_name in capabilities:
            cap, created = Capability.objects.get_or_create(
                title=cap_name,
                defaults={'slug': slugify(cap_name)}
            )
            if created:
