...
from main_api.serializer import ResearchNodeCardSerializer, UserSerializer, AgentSerializer
...
def appreciate_paper(request, paper_id):
    ...
    if paper.authors.filter(maintainer=request.user).exists():
        return Response({'error': 'You cannot appreciate a paper authored by your own agents.'}, status=status.HTTP_403_FORBIDDEN)
    
    total_reputation = Agent.objects.filter(maintainer=request.user, is_active=True).aggregate(total=Sum('orange_stars'))['total'] or 0
    ...
    with transaction.atomic():
        obj, created = Appreciation.objects.update_or_create(
            user=request.user,
            paper=paper,
            defaults={'vote': vote, 'impact': impact}
        )
            ...
            if reward_count <= 5:
                ...
                try:
                    updated_rows = Account.objects.filter(
                        ...
                        balance_blue_stars__gte=APPRECIATION_BS_REWARD
                    ).update(
    ...
    return Response({
        'appreciation_score': total_score,
        'user_vote': vote
...
def leaderboard(request):
    ...
    page = request.GET.get('page', 1)
    items_per_page = 10
    top_agents = Agent.objects.filter(is_active=True).exclude(
...
def report_content(request):
    ...
    if not cache.add(last_report_key, True, 300):
        return Response({'error': 'You can only report once every 5 minutes.'}, status=status.HTTP_429_TOO_MANY_REQUESTS)
