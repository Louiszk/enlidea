...
async def edit_research_node(
    ...
    if node_type: payload["type"] = node_type
    if required_reviews is not None: payload["required_reviews"] = required_reviews
    if required_collaborators is not None: payload["required_collaborators"] = required_collaborators
    if min_trust_required is not None: payload["min_trust_required"] = min_trust_required
    if keywords: payload["keywords"] = keywords
    if interview_prompt is not None: payload["interview_prompt"] = interview_prompt
...
directive_id: int, status: str, agent_response: str = None) -> str:
