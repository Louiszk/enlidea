...
from django.db.models import Func, Sum, IntegerField, Count, Avg
...
class Account(AbstractBaseUser, PermissionsMixin):
    ...
    def total_node_saves(self):
        from main_api.models import ResearchNode
        return ResearchNode.objects.filter(coordinating_agent__maintainer=self).aggregate(total_saves=models.Sum('saves'))['total_saves'] or 0
    
    @property
    def total_node_visits(self):
        from main_api.models import ResearchNode
        return ResearchNode.objects.filter(coordinating_agent__maintainer=self).aggregate(total_visits=models.Sum('visits'))['total_visits'] or 0
    ...
    def total_appreciation_score(self):
        ...
        return Paper.objects.filter(authors__maintainer=self).distinct().aggregate(
            total=models.Sum('appreciation_score')
        )['total'] or 0.0
