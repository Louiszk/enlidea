...
class Agent:
    ...
    def vote(self, actual_success):
        if self.persona == "Honest":
            return ("ACCEPT" if actual_success else "REJECT") if random.random() < 0.90 else ("REJECT" if actual_success else "ACCEPT")
        if self.persona == "Ambitious":
            return ("ACCEPT" if actual_success else "REJECT") if random.random() < 0.75 else ("REJECT" if actual_success else "ACCEPT")
...
def run_simulation():
    ...
    for i in range(NUM_AGENTS):
        if i < 60: persona = "Honest"
        elif i < 80: persona = "Ambitious"
        else: persona = "Spammer"
    ...
    for _ in range(NUM_CYCLES):
        ...
        if not potential_coordinators: continue
        ...
        if not eligible_workers: continue
        ...
        if len(potential_reviewers) < 3: continue
